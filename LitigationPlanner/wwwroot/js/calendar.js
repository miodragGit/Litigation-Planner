...
function GetLitigationsDates(calendar) {
    $.ajax({
        type: "GET",
        url: "/Litigation/GetLitigationsDates",
        contentType: 'application/json',
        success: function (response) {
            var dates = response;

            UpdateCalendarOnSuccess(dates, calendar);
        },
        error: function (response) {

            UpdateCalendarOnError(calendar);
        }
        
    });
}

function UpdateCalendarOnSuccess(dates, calendar) {

    document.getElementById(calendar.id + '-year').innerHTML = calendar.date.getFullYear();
    document.getElementById(calendar.id + '-month').innerHTML = months[calendar.date.getMonth()];

    for (i = 1; i <= 42; i++) {
        document.getElementById(calendar.id + '-day-num-' + i).innerHTML = "";
        document.getElementById(calendar.id + '-day-' + i).className = calendar.id + " day";

    }

    firstDay = new Date(calendar.date.getFullYear(), calendar.date.getMonth(), 1).getDay();
    lastDay = new Date((calendar.date.getMonth() + 1 > 11) ? calendar.date.getFullYear() + 1 : calendar.date.getFullYear(), (calendar.date.getMonth() + 1 > 12) ? 0 : calendar.date.getMonth() + 1, 0).getDate();

    previousLastDay = new Date((calendar.date.getMonth() < 0) ? calendar.date.getFullYear() - 1 : calendar.date.getFullYear(), (calendar.date.getMonth() < 0) ? 11 : calendar.date.getMonth(), 0).getDate();

    if (firstDay != 0)
        for (i = 0, j = previousLastDay; i < calendar.label.indexOf(label[firstDay]); i++ , j--) {
            document.getElementById(calendar.id + '-day-num-' + (1 + i)).innerHTML = j;
            document.getElementById(calendar.id + '-day-' + (1 + i)).className = calendar.id + " day diluted";
        }

    for (i = 1; i <= lastDay; i++) {
        document.getElementById(calendar.id + '-day-num-' + (calendar.label.indexOf(label[firstDay]) + i)).innerHTML = i;

        for (k = 0; k < dates.litigationsDates.length; k++) {
            if (i == dates.litigationsDates[k]) {
                document.getElementById(calendar.id + '-day-num-' + (calendar.label.indexOf(label[firstDay]) + i)).style.backgroundColor = calendar.colors[0];
            }
            else {
                continue;
            }
        ...
        if (i == calendar.date.getDate())
            document.getElementById(calendar.id + '-day-radio-' + (calendar.label.indexOf(label[firstDay]) + i)).checked = true;
    }

    for (i = lastDay + 1, j = 1; (calendar.label.indexOf(label[firstDay]) + i) <= 42; i++ , j++) {
        document.getElementById(calendar.id + '-day-num-' + (calendar.label.indexOf(label[firstDay]) + i)).innerHTML = j;
        document.getElementById(calendar.id + '-day-' + (calendar.label.indexOf(label[firstDay]) + i)).className = calendar.id + " day diluted";
    }
}

function UpdateCalendarOnError(calendar) {
